...
DELIVERY_JSON = SCRIPT_DIR / "delivery_data.json"
DISPATCH_JSON = SCRIPT_DIR / "dispatch_data.json"
VENUES_JSON = SCRIPT_DIR / "venues_data.json"
    ...
    try:
        ...
        if tt_total > 0:
            ...
            out.append(Result("delivery:weekly_vs_contract_tt",
                              "PASS" if ok_tie else "FAIL",
                              f"{diff_pct:.4f}", "0.1",
                              f"contractWeekly ${cw_total:,} vs sum(tt) ${tt_total:,}"
                              if not ok_tie else ""))
    except Exception as e:
    ...
    try:
        ...
        if n_eq + n_ne >= 100:
            ...
            out.append(Result("dispatch:rev_equals_contract_rental",
                              "PASS" if ok_rev else "FAIL",
                              f"{pct:.2f}", "98",
                              "" if ok_rev else f"{n_ne} stops where stop.rev != contractInfo.tt — rental semantics drifted"))
    except Exception as e:
